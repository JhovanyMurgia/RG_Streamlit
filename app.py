    ...
    if uploaded_file is not None:
        ...
        if st.button("Executar Modelos", key="btn-cadunico"):
            with st.spinner("Executando modelo..."):
                ...
                # Mostrar os dados extraídos
                if isinstance(data, dict):
                    # Verifica se algum valor do dicionário é uma lista
                    if any(isinstance(v, list) for v in data.values()):
                        # Garante que todos os valores sejam listas de mesmo comprimento
                        ...
                        # Caso todos os valores sejam escalares
     ...
     # Converter PDF em imagens e exibir
        ...
        if st.button("Executar Modelos", key="btn-comp_res"):
            with st.spinner("Executando modelo..."):
                ...
                # Mostrar os dados extraídos
                if isinstance(data, dict):
                    # Remove as chaves com valores nulos, vazios ou None
                    dados_filtrados = {k: v for k, v in data.items() if v not in [None, '', [], {}, 'null']}

                    if dados_filtrados:
                        df = pd.DataFrame(list(dados_filtrados.items()), columns=["Campos no documento", "Resultado identificado"])
                        st.table(df)
                    else:
                        st.warning("Nenhum dado identificado no documento.")
                else:
